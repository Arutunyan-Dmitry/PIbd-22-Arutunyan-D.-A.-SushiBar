...
namespace SushiBarFileImplement.Implements
{
    public class DishStorage : IDishStorage
    {
        ...
        {
            return source.Dishes.Select(CreateModel).ToList();
        }
        public List<DishViewModel> GetFilteredList(DishBindingModel model)
        {
            ...
            return source.Dishes.Where(rec => rec.DishName.Contains(model.DishName))
            .Select(CreateModel)
            .ToList();
        }
        ...
        private DishViewModel CreateModel(Dish dish)
        {
            return new DishViewModel
            {
                ...
                DishIngredients = dish.DishIngredients
                    ...
                    (source.Ingredients.FirstOrDefault(recC => recC.Id == recPC.Key)
                    ?.IngredientName, recPC.Value))
            };
        }
    }
}
